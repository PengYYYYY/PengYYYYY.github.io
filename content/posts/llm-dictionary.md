---
title: "大模型基础名词词典"
description: "一份按主题整理的大模型基础名词词典：模型本体、输入输出、生成过程、训练阶段，以及 Agent 相关概念。"
summary: "一份按主题整理的大模型基础名词词典：模型本体、输入输出、生成过程、训练阶段，以及 Agent 相关概念。"
...
### 权重（Weights / Parameters） {#权重-weights-parameters}
...
### 基座模型（Base / Pretrained Model） {#基座模型-base-pretrained-model}
- **是什么**：大规模预训练后、还没做指令微调的模型。只会"续写文本"，不理解"这是一道题、该给简洁答案"。

### 指令微调模型（Instruction-tuned / Instruct Model） {#指令微调模型-instruction-tuned-instruct-model}
- **是什么**：在基座模型上，用大量"（指令，理想回答）"对继续训练（SFT），再用人类反馈对齐（RLHF），让它学会"听懂指令、好好回答"。

### Transformer / 注意力机制（Attention） {#transformer-注意力机制-attention}
...
### 自回归生成（Autoregressive Generation） {#自回归生成-autoregressive-generation}
- **是什么**：一个一个吐 token，每一步都基于前面所有 token 预测"下一个最可能的词"。

### 推理（Inference） {#推理-inference}
- **是什么**：用训练好的权重跑前向计算、产出结果。区别于"训练"。

### 训练 / 预训练 / 微调（Training / Pretraining / Fine-tuning） {#训练-预训练-微调-training-pretraining-fine-tuning}
...
### 温度（Temperature）/ Top-p / 采样 {#温度-temperature-top-p-采样}
...
## 六、推理的硬件视角：GPU / HBM / KV Cache {#六、推理的硬件视角-gpu-hbm-kv-cache}
...
### HBM（高带宽显存 / High Bandwidth Memory） {#hbm-高带宽显存-high-bandwidth-memory}
...
### 深入：Prefill vs Decode（为什么首 token 和后续 token 不一样） {#深入-prefill-vs-decode-为什么首-token-和后续-token-不一样}
...
## 七、应用层：Agent 与工具调用 {#七、应用层-agent-与工具调用}
...
### 对齐与安全（护栏 / 提示注入） {#对齐与安全-护栏-提示注入}
- **是什么**：用输入/输出过滤、拒答、护栏等手段，防模型被"提示注入"（在 prompt 里塞指令骗它干坏事）或被诱导输出有害内容。

### 基准测试（Benchmark，如 MMLU） {#基准测试-benchmark-如-mmlu}
